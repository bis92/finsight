...
| 용도 | 값 |
|------|------|
| 수입·절감 | `#05b169` (텍스트 색 전용, fill 금지) |
| 지출·경고 | `#cf202f` (텍스트 색 전용, fill 금지) |
| 액센트(링크·CTA·워드마크) | **Coinbase Blue `#0052ff`** (press `#003ecc`) — **보라/인디고 금지** |
| 보더(hairline) | `#dee1e6` |

- 액센트는 **단일 브랜드 색 `#0052ff`**만 쓴다(CTA·워드마크·인라인 링크에 scarce하게). 제2 브랜드 색 도입 금지, 보라/인디고 금지.
- 차트/카테고리 팔레트는 `#0052ff` 기반 채도 낮은 파란 계열, 인접 조각은 명도로 구분. 상세 팔레트는 `DESIGN.md` 참조.
- ※ 색상 원천은 `DESIGN.md`로 확정됨(디자인 원본 유지). 이전 teal 지정은 이 값으로 대체.
