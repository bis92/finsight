...
> 계획의 단일 원천은 `plan.md`(§9)다. 상충 시 plan.md가 우선한다. **Light 서피스 기본**(다크모드는 출시 후).

## 디자인 원칙

1. **도구처럼 보인다** — 마케팅 페이지가 아니라 매일 여는 금융 대시보드. 신뢰·가독성이 최우선.
2. **숫자가 주인공** — 금액·비율·차트를 먼저 보여준다.
3. **Light 서피스 기본** — 흰 배경 + border로 구분. 다크모드는 로드맵.
